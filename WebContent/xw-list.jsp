	...
	<body>
		...
		<div>
			<!-- <div class="search-big">
				...
				%>
				<form action="xinwen_query.action" method="post">
					<input type="text" value="${para.timu }" placeholder="请输入新闻题目" name="timu"/>
					<label>新型职业农民:</label>
					<%if(type==2){ %>
					...
					<a href="count.action?i=-1&type=xw">添加新闻</a>
					<%} %>
				</form>
			    </div>
			</div>
			...
			<table class="table table-striped table-hover table-bordered">
				...
				%>
				<%if(xinwen.getNumber()!=0){
				for(int i=0;i<xinwen.getPlist().size();i++){ 
					Xinwen xw=xinwen.getPlist().get(i);%>
				<tr>
					<td><%=i+1 %></td>
					<td><%=xw.getXinwen_timu() %></td>
					<td><%=xw.getXinwen_gengxin_riqi() %></td>
					
					<%if(xw.getXinwen_biaozhiwei()==2){ %>
					<td>新型农民</td>
					<%}else{ %>
					<td>其他</td>
					<%} %>
					...
					<%if(session.getAttribute("user_leixing").equals("2")||session.getAttribute("user_leixing").equals("3")){ %>
					<td>
					<%if(xw.getXinwen_biaozhiwei()==2){//是新型 %>
					<a href="xinwen_addNew.action?i=<%=i+1%>">取消新型</a>&nbsp;/&nbsp;
					<%}else{ %>
					<a href="xinwen_addNew.action?i=<%=i+1%>">添加新型</a>&nbsp;/&nbsp;
					<%} %>
						<a href="count.action?i=<%=i+1%>&type=xw">修改</a>&nbsp;/&nbsp;
						<%if(xw.getXinwen_zhidingwei()==0){ %>
						<a href="xinwen_zhiding.action?i=<%=i+1%>">置顶</a>&nbsp;/&nbsp;
						<%}else{ %>
						<a href="xinwen_cancelzhiding.action?i=<%=i+1%>">取消置顶</a>&nbsp;/&nbsp;
						<%} %>
						<a href="xinwen_delete.action?i=<%=i+1%>" onclick= 'return confirm( "确定要删除吗? ") '>刪除</a>
					</td>
					<%} %>
				</tr>
				<%}} %>
       ...
       <script type="text/javascript">
		...
		<form method="post" id="ds" action="xinwen_query.action">
      	  ...
      	  <tr>
						<td colspan="2" align="right" height="20"  nowrap class="textbar3" >
							<input type="button" onclick="doSubmit(1)" value="首页">
      						<input type="button" onclick="doSubmit(<%=xinwen.getNowpage()-1 %>)" value="上一页">
      						...
      						<input type="button" onclick="doSubmit(<%=xinwen.getNowpage()+1 %>)" value="下一页">&nbsp; 
      						<input type="button" onclick="doSubmit(<%=xinwen.getSumPage() %>)" value="尾页">&nbsp; &nbsp;
							 第<%=xinwen.getNowpage() %>/<%=xinwen.getSumPage() %>页 &nbsp;      				
							&nbsp; 共<%=xinwen.getNumber()%>条 &nbsp;
						</td>
          		</tr>
        </table>
        </form>
