	...
	<body>
		...
		<div>
			<!-- <div class="search-big">
				<div class="search"> -->
				<form action="jgtb_query.action" method="post">
					<input type="text" value="${para.timu }" placeholder="请输入价格通报题目" name="timu"/>
					<input type="submit" value="查找" />
					<%if(session.getAttribute("user_leixing").equals("2")||session.getAttribute("user_leixing").equals("3")){ %>
					<a href="count.action?i=-1&type=jg">添加</a>
					<%} %>
					</form>
			    </div>
			</div>
			...
			<table class="table table-striped table-hover table-bordered">
				...
				<%if(jgtb.getNumber()!=0){
				for(int i=0;i<jgtb.getPlist().size();i++){ 
					Jiage_tongbao jg=jgtb.getPlist().get(i);%>
				<tr>
					<td><%=i+1 %></td>
					<td><%=jg.getJiage_tongbao_yuefen() %></td>
					<td><%=jg.getJiage_tongbao_timu() %></td>
					<td><%=jg.getJiage_tongbao_chuanjian_shijian() %></td>
					<%if(session.getAttribute("user_leixing").equals("2")||session.getAttribute("user_leixing").equals("3")){ %>
					<td>
					<!-- change by Li Zhiying -->
						<a href="count.action?i=<%=i+1%>&type=jg">修改</a>&nbsp;/&nbsp;
					<!-- change by Li Zhiying -->
						<a href="jgtb_delete.action?i=<%=i+1%>" onclick= 'return confirm( "确定要删除吗? ") '>刪除</a>
					</td>
					<%} %>
				</tr>
				<%}} %>
		...
		<form method="post" id="ds" action="jgtb_query.action">
      	  ...
      	  <tr>
						<td colspan="2" align="right" height="20"  nowrap class="textbar3" >
							<input type="button" onclick="doSubmit(1)" value="首页">
      						<input type="button" onclick="doSubmit(<%=jgtb.getNowpage()-1 %>)" value="上一页">
      						...
      						<input type="button" onclick="doSubmit(<%=jgtb.getNowpage()+1 %>)" value="下一页">&nbsp; 
      						<input type="button" onclick="doSubmit(<%=jgtb.getSumPage() %>)" value="尾页">&nbsp; &nbsp;
							 第<%=jgtb.getNowpage() %>/<%=jgtb.getSumPage() %>页 &nbsp;      				
							&nbsp; 共<%=jgtb.getNumber()%>条 &nbsp;
						</td>
          		</tr>
        </table>
